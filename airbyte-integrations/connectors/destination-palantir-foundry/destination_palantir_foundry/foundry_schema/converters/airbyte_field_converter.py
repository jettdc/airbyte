...
DEFAULT_FOUNDRY_FIELD_DESCRIPTION = FoundryFieldDescription(field_schema=StringFieldSchema, custom_metadata=IS_JSON)


def _get_ab_type_from_arr(types: List[str]) -> Optional[str]:
    types_without_null = list(filter(lambda field_type: field_type != "null", types))
...
class AirbyteToFoundryConverter:
    type_mappings: Dict[AirbyteFieldDescription, FoundryFieldDescription] = {
        ...
        AirbyteFieldDescription(type_name="object", obj_is_union=True): \
            FoundryFieldDescription(field_schema=StringFieldSchema, custom_metadata=IS_JSON),
    ...
    def convert_ab_to_foundry_field_schema(ab_field_name: Optional[str], ab_field_schema: Dict[str, Any]) -> FoundryFieldSchema:
        ...
        ab_field_type: Union[str, List[str], None] = ab_field_schema.get("type", None)
        ...
        arr_is_union = None if items_type is None else isinstance(items_type, list)

        obj_is_union = None if ab_field_schema.get("oneOf", None) is None else True
        ...
        if foundry_field_description.field_schema == ArrayFieldSchema:
            ...
            if item_type is not None and not isinstance(items_type, list):
                foundry_field_schema_kwargs["arraySubtype"] = AirbyteToFoundryConverter.convert_ab_to_foundry_field_schema(None, item_type)
            else:
            ...
            for obj_property_name, obj_property_schema in ab_field_schema.get("properties", {}).items():
                sub_schemas.append(AirbyteToFoundryConverter.convert_ab_to_foundry_field_schema(obj_property_name, obj_property_schema))
        ...
        if ab_description is not None:
            custom_metadata = foundry_field_schema_kwargs.get("customMetadata", {})
            foundry_field_schema_kwargs["customMetadata"] = {**custom_metadata, "ab_description": ab_description}
    ...
    def convert_record_field(value: Any, field_schema: FoundryFieldSchema) -> Any:
        ...
        if isinstance(field_schema, ArrayFieldSchema):
            if not isinstance(value, list):
                raise ValueError(f"Cannot parse nonlist array type: {field_schema.name}")
            ...
            if not isinstance(value, bool):
                raise ValueError(f"Cannot parse nonbool type to bool: {field_schema.name}")

            return value

        elif isinstance(field_schema, DateFieldSchema):
            if not isinstance(value, str):
                raise ValueError(f"Cannot parse nonstring date type: {field_schema.name}")

            if value.endswith("BC"):
                raise ValueError(f"Cannot parse BC date type: {field_schema.name}")
            ...
            if not isinstance(value, str):
                raise ValueError(f"Cannot parse nonstring string type: {field_schema.name}")

            return value

        elif isinstance(field_schema, StructFieldSchema):
            if not isinstance(value, dict):
                raise ValueError(f"Cannot parse nonobject struct type: {field_schema.name}")
            ...
            if not isinstance(value, str):
                raise ValueError(f"Cannot parse nonstring timestamp type: {field_schema.name}")
